...
"""
def index_transform(index, vec):
    '''
    Return the corresponding vector given the index map 
    e.g. [3, 1, 2] maps 1st to 3rd, 2nd to 1st, 3rd to 2nd
    '''
    n = len(vec)
    nvec = np.zeros((n, 1))
    for i in range(n):
        nvec[index[i], 1] = vec[i]
    return nvec 

def get_bk_index_map(n):
    '''
    Return the index map of bk given number of nodes (n)
    '''
