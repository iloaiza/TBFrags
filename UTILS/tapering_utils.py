# Obtained from https://github.com/quantumlib/OpenFermion/blob/v1.3.0/src/openfermion/transforms/repconversions/qubit_tapering_from_stabilizer.py#L404-L491 on 7.3.2022.
...
def fix_single_term(term, position, fixed_op, other_op, stabilizer):
    ...
    if (position, fixed_op) in pauli_tuple or (position,
        ...
        out = term
    if (fixed_op == 'Z'):
       second_unitary = QubitOperator(term = (position, 'Z'), coefficient = 1/numpy.sqrt(2)) +  QubitOperator(term = (position, 'X'), coefficient = 1/numpy.sqrt(2))
       out = second_unitary * out * second_unitary
    return out
...
def _reduce_terms(terms, stabilizer_list, manual_input, fixed_positions):
    ...
    for i, _ in enumerate(stabilizer_list):
        ...
        if fixed_op == 'Z' and other_op == 'Y':
           first_unitary = QubitOperator(term = (fixed_positions[i], 'X'), coefficient = 1/numpy.sqrt(2)) + 1/numpy.sqrt(2) * stabilizer_list[0]
           second_unitary = QubitOperator(term = (fixed_positions[i], 'Z'), coefficient = 1/numpy.sqrt(2)) +  QubitOperator(term = (fixed_positions[i], 'X'), coefficient = 1/numpy.sqrt(2))
           unitary.append(second_unitary * first_unitary)
        else:
           unitary.append(QubitOperator(term = (fixed_positions[i], 'Z'), coefficient = 1/numpy.sqrt(2)) + 1/numpy.sqrt(2) * stabilizer_list[0])
    ...
    return terms, fixed_positions, unitary
...
def reduce_number_of_terms(operator,
    ...
    if maintain_length:
        ...
        (reduced_operator,
         fixed_positions, unitary) = _reduce_terms(operator, stabilizer_list,
                                          manual_input, fixed_positions)

    if output_fixed_positions:
        return reduced_operator, fixed_positions, unitary
    else:
        return reduced_operator, unitary
...
def taper_off_qubits(operator,
    ...
    (ham_to_update,
     qbts_to_rm, unitary) = reduce_number_of_terms(operator,
                                          stabilizers,
                                          maintain_length=False,
    ...
    if output_tapered_positions:
        return skimmed_operator, removed_positions, unitary
    else:
        return skimmed_operator, unitary

def wf_removed_qubit(wf, position, n_qubits):
   new_wf = numpy.zeros(2 ** (n_qubits - 1), dtype = wf.dtype)
   n_segment = 2 ** (position)
   n_in_each_segment = 2 ** (n_qubits - position - 1)
   count = 0
   for i in range(n_segment):
      start_num = i * (n_in_each_segment * 2)
      for j in range(n_in_each_segment):
         new_wf[count] = wf[start_num + j]
         count += 1
   return new_wf
